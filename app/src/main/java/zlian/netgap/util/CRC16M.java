...
public class CRC16M {
    ...
    private static byte[] TABLE_Hi = { 0x00, (byte) 0xC1, (byte) 0x81,
    ...
    private static byte[] TABLE_Lo = { (byte) 0x00, (byte) 0xC0, (byte) 0xC1,
    ...
    public void update(byte[] data, int len) {
        int uIndex; // int i = 0;
        for (int i = 0; i < len; i++) {
            uIndex = (CRCHi ^ data[i]) & 0xff;
            CRCHi = (byte) ((CRCLo ^ TABLE_Hi[uIndex])& 0xff);
            CRCLo = TABLE_Lo[uIndex];
        }
        value = (((CRCHi & 0x000000ff ) << 8 | ( CRCLo & 0xff))) & 0xffff;
        return;
    }

    public void update2(byte[] data, int len) {
        int index; // int i = 0;
        for (int i = 0; i < len; i++) {
            index = (CRCLo ^ data[i])  & 0xff;
            CRCLo = (byte) ((CRCHi ^ TABLE_Hi[index]) & 0xff);
            CRCHi = TABLE_Lo[index];
        }
        value = (((CRCHi & 0x000000ff )<< 8 )| ( CRCLo & 0xff)) ;
        return;
    }
